...
// -----------------------------working code 25-05---groom-----------
                  $getProposedTo = DB::table('proposal_request')->select('proposed_to','proposed_to_role')
                    ->where('proposed_by_groom',$groom_profile_id)
                  ->where('proposed_by_role',2)
                        ->get();
                foreach ($getProposedTo as $keyReq => $valueReq) {
                  $getProposedToValue[] = $valueReq->proposed_to;
                }
               
                // if($getProposedToValue) {
                 $proposal_request_status = DB::table('proposal_request')->select('proposel_status')
                  ->where('proposed_by_groom',$groom_profile_id)
                  ->where('proposed_by_role',2)
                  ->whereIn('proposed_to',$getProposedToValue)
                  ->first();
                // dd('proposal_request_status1',$proposal_request_status);

                // }

                $incommingReqStatus = DB::table('proposal_request')->select('proposel_status')
                // ->whereIn('proposed_by_groom',$incommingProReq)
                 ->where('proposed_to',$groom_profile_id)
                ->where('proposed_to_role',2)
                ->where('proposel_status',1)
                ->whereIn('proposed_by_groom',$incommingProReq)
                ->first();
                // dd('incommingReqStatus',$incommingReqStatus);

// ----------------------------end-working code 25-05--------------
    ...
    // -------------proposal_requestsend------working code 25-05---bride-----------
                $getProposedTo = DB::table('proposal_request')->select('proposed_to','proposed_to_role')
                  ->where('proposed_by_bride',$bride_profile_id)
                  ->where('proposed_by_role',3)
                        ->get();
                ...
                $getProposedByGroomId = DB::table('proposal_request')->select('proposed_by_groom','proposed_by_role')
                   ->where('proposed_to',$bride_profile_id)
                 ->where('proposed_to_role',3)
                 ->where('proposel_status',1)->first();

                if(isset($getProposedByGroomId)){
                  $incommingReqStatus = DB::table('proposal_request')->select('proposel_status')
                  // ->whereIn('proposed_by_groom',$incommingProReq)
                   ->where('proposed_to',$bride_profile_id)
                  ->where('proposed_to_role',3)
                  ->where('proposel_status',1)
                  ->where('proposed_by_groom',$getProposedByGroomId->proposed_by_groom)
                  ->first();
                }
        ...
        // dd('incommingReqStatus bride',$incommingReqStatus);
    // ---------------------------end--working code 25-05---bride-----------
    ...
    ?>
        ...
        @foreach($brideGroomFilterData as $key=>$value)
           <div class="jobs-item with-thumb">
            ...
            <!-- ------------------buttons---if conditions- -->
                @if(isset($incommingReqStatus))  

                    <a  class="photo_view"  id="proposal_request">Proposal Accepted</a>
                @else      
                    @if(isset($proposal_request_status))
                           ...
                           @if($proposal_request_status->proposel_status == 0)
                            ...
                            @elseif($proposal_request_status->proposel_status ==1)
                                ...
                                <a  class="photo_view"  id="proposal_request">Proposal Accepted</a>
                            @else
                                 <a onClick="proposalRequest({{$value}})" class="photo_view"  id="proposal_request">Proposal Request</a>
                               

                            @endif  
                         @else
                         <a onClick="proposalRequest({{$value}})" class="photo_view"  id="proposal_request">Proposal Requestori</a>
                    @endif
